...
The **MediaOcean** and **Reports** tabs are visible to every role and used across agencies, so their data is partitioned by agency and the partition is enforced by security rules, not the UI. Admin/Exec read every agency; everyone else reads the agencies their email domain maps to — checked **live** against the `agencies` ↔ domains mapping (and company-wide domains), so a mapping change applies to everyone with no sync — `∪ assignedAgencies ∪ clientAgencies` (`resolveAgencyScope` in `lib/format/agency-scope.ts`, mirrored by `canReadAgencyData` in `firestoreRules.txt` and `canReadAgency` in `storage.rules` — keep all three in sync). Rows whose agency isn't one of the app agencies are tagged `_unassigned` (Admin/Exec only).
