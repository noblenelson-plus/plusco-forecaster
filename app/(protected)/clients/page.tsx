...
import type { ClientStatus } from "../../../lib/constants/client.constants";export default function ClientsPage() {
const { profile, isAdmin } = useUserProfile();const [clients, setClients] = useState<Client[]>([]);
const [loading, setLoading] = useState(true);
const [error, setError] = useState("");// Filter state
const [search, setSearch] = useState("");
const [statusFilter, setStatusFilter] = useState<"ALL" | ClientStatus>("ALL");// Drawer state
const [drawerOpen, setDrawerOpen] = useState(false);
const [editingClient, setEditingClient] = useState<Client | null>(null);// Fetch clients
useEffect(() => {
if (!profile) return;async function fetchClients() {
  setLoading(true);
  setError("");
  try {
    let snapshot;    if (isAdmin) {
...
}, [profile, isAdmin]);// Filtered clients
